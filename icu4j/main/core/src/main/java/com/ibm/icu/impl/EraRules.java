...
public class EraRules {
    ...
    private int[] eraCodes;
    private int numEras;
    private int currentEraStartIndex;

    private EraRules(int[] startDates, int[] eraCodes, int numEras) {
        this.startDates = startDates;
        this.eraCodes = eraCodes;
    ...
    public static EraRules getInstance(CalType calType, boolean includeTentativeEra) {
        ...
        int[] eraCodes = new int[numEras];
        int startDateIdx = 0;
        int numErasNonTentative = numEras;
        for (int eraIdx = 0; eraIdx < eraStartDates.size(); eraIdx++) {
            if (isSet(eraStartDates.get(eraIdx).intValue())) {
                if (eraIdx >= firstTentativeIdx) {
                    numErasNonTentative = startDateIdx;
                }
                startDates[startDateIdx] = eraStartDates.get(eraIdx).intValue();
        ...
        if (firstTentativeIdx < Integer.MAX_VALUE && !includeTentativeEra) {
            return new EraRules(startDates, eraCodes, numErasNonTentative);
        }

        return new EraRules(startDates, eraCodes, numEras);
    }
    ...
    public int[] getStartDate(int eraCode, int[] fillIn) {
        ...
        for (int startIndex = numEras; startIndex > 0;) {
            if (eraCodes[--startIndex] == eraCode) {
                return decodeDate(startDates[startIndex], fillIn);
            }
        }
    ...
    public int getStartYear(int eraCode) {
        ...
        for (int startIndex = numEras; startIndex > 0;) {
            if (eraCodes[--startIndex] == eraCode) {
                int[] fields = decodeDate(startDates[startIndex], null);
                return fields[0];
            }
        }
        // We did not find the requested eraCode in our data
